...
function translateLang(phrase, toLang) {
  ...
  request.addEventListener("loadend", function () {
    ...
    debugger;
    if (this.status === 200) {
      printElement(json, phrase, toLang);
    } else {
      printError(this, json, phrase, toLang);
    }
  });
...
function getLangOptions(phrase, toLang) 
{
  const request = new XMLHttpRequest();
  request.withCredentials = true;
  request.addEventListener("loadend", function () {
    ...
    debugger;
    if (this.status === 200) {
      fillOutSelectOptions(jsonResponse);
    } else {
      printError(this, json, phrase, toLang);
    }
  });

  request.open("GET", "https://google-translate1.p.rapidapi.com/language/translate/v2/languages?target=en", true);
  // request.setRequestHeader("Accept-Encoding", "application/gzip");
  request.setRequestHeader("X-RapidAPI-Key", `${process.env.API_KEY}`);
  request.setRequestHeader("X-RapidAPI-Host", "google-translate1.p.rapidapi.com");
...
function handleFormSubmission(event) {
  ...
  document.getElementById('translateText').reset();
  translateLang(phrase, toLang);
}

function printElement(jsonResponse, phrase, toLang) {
  let spot = document.querySelector(".showTranslated");
  spot.innerText = jsonResponse.data.translations[0].translatedText;
}
